...
gender(male_ind)=2; % set Males to 2
...
computer(pc_ind)=1; % set PC to 1
apple_ind=find(strcmp(data(:, pc_header), 'Apple'));
computer(apple_ind)=2; % set Apple to 2
...
stackedBar=[women_pc women_apple; men_pc men_apple];
a3=subplot(2,2,3);
...
gaming_header=find(strcmp(headers,'games_hours'));

subplot(1,3,2)
plot([data{:,gaming_header}],[data{:,grade_header}],'ko');
xlabel('Gaming Hours')
ylabel('grade')
...
sleep_hours_header=find(strcmp(headers,'sleep'));

subplot(1,3,3)
plot([data{:,gaming_header}],[data{:,sleep_hours_header}],'ko');
xlabel('Gaming Hours')
...
figure(3)
scatter3([data{:,sleep_hours_header}],[data{:,gaming_header}],[data{:,facebook_header}]);
...
gpa_header=find(strcmp(headers,'GPA_UW'));
drink_header=find(strcmp(headers,'drink'));

figure(4)
plot([data{:,drink_header}],[data{:,gpa_header}],'ko');
xlabel('Drinking Occasions')
ylabel('GPA')
